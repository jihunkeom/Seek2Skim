...
def masked_softmax(vec, mask, dim=-1, eps=1e-6):
    if mask.dim() == 2:
        ...
        mask = mask[:, None, :, :]
    assert mask.dim() == 4, "Invalid skim mask shape"
...
def masked_softmax2(vec, mask, dim=-1):
    ...
    vec.masked_fill((mask < 1.), -1e9)
    return F.softmax(vec.float(), dim=dim)

def masked_softmax3(vec, mask, dim=1, eps=1e-6):
    mask = mask[:, None, None, :]
    exps = torch.exp(vec)
    ...
    def forward(
        ...
        if self.training and skim_mask is not None:
            if not self.is_decoder:
                attn_weights = masked_softmax3(scores.float(), skim_mask, dim=3)
            else:
...
class T5ForConditionalGenerationSeek2SkimV2(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        if self.training:
            ...
            for i in range(self.config.num_layers - 1):
                ...
                if i < self.decoder.num_skimmers:
            ...
            decoder_skim_loss /= self.decoder.num_skimmers
            cross_skim_loss /= (num_decoder_layers - 1)
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # print(f"Eval Decoder Skim Info: {decoder_skimmed_lens}")
