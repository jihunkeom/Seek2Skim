...
class T5Trainer(Trainer):
    ...
    def evaluate(self, eval_datasets: Optional[Dict[str, Dataset]] = None) -> Dict[str, float]:
        ...
        for eval_task, eval_dataset in eval_datasets.items():
            ...
            tasks_metric["speedup"] = round(1 / (self.model.relative_flops / self.model.num_instances), 5)
            ...
            self.model.reset_ee_stats()

        # Computes the average metrics across all the tasks without their corresponding losses.
        metrics = [results[key] for key in results.keys() if ("loss" not in key and "Time" not in key and "exit" not in key and "flops" not in key and "speed" not in key)]
